...
// 后端接口
live2d_settings['modelAPI']             = '/live2d_api/model/';   // 部分资源从github下载到本地了
...
function initModel(waifuPath, type) {
    ...
    $.ajax({
        cache: true,
        url: '/live2d_models.json',
        dataType: "json",
        success: function(result) {
            window.live2d_models = result;
            initModelInternal(waifuPath, type);
        },
        error: function() {
        }
    });
...
function initModelInternal(waifuPath, type) {
    ...
    /* 判断 JQuery */
    if (typeof($.ajax) != 'function') typeof(jQuery.ajax) == 'function' ? window.$ = jQuery : null;
    
    /* 加载看板娘样式 */
    if (typeof live2d_settings.waifuSize === 'object' && Array.isArray(live2d_settings.waifuSize)) {
    } else {
        live2d_settings.waifuSize = live2d_settings.waifuSize.split('x');
    }
    
    if (typeof live2d_settings.waifuTipsSize === 'object' && Array.isArray(live2d_settings.waifuTipsSize)) {
    } else {
        live2d_settings.waifuTipsSize = live2d_settings.waifuTipsSize.split('x');
    }
    
    if (typeof live2d_settings.waifuEdgeSide === 'object' && Array.isArray(live2d_settings.waifuEdgeSide)) {
    } else {
    ...
    $('.waifu-tool .fui-info-circle').click(function (){   // 没用到
        window.open(live2d_settings.aboutPageUrl);
    });
    
    if (typeof(waifuPath) == "object") loadTipsMessage(waifuPath); else {
        // 检查是否有全局的 waifuTipsData（来自 autoload.js）
        if (typeof window.waifuTipsData !== 'undefined') {
            loadTipsMessage(window.waifuTipsData);
        } else {
            $.ajax({
                cache: true,
...
function loadModel(modelId) {
    ...
    var model = window.live2d_models.models[modelId];
...
function loadTipsMessage(result) {
    ...
    window.showWelcomeMessage = function(result) {
        ...
        else if (now > 11 && now <= 12) text = getRandText(result.waifu.hour_tips['t11-12']);
        else if (now > 12 && now <= 15) text = getRandText(result.waifu.hour_tips['t12-15']);
        else if (now > 15 && now <= 17) text = getRandText(result.waifu.hour_tips['t15-17']);
    ...
    function loadOtherModel() {
        ...
        var models = Object.keys(window.live2d_models.models);
        var currentIndex = models.indexOf(modelId.toString());
        ...
        var newModelId = models[currentIndex];
        loadModel(newModelId);
        
        var message = window.live2d_models.models[newModelId].message;
